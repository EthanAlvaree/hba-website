...
import { getStudentDetail } from "@/lib/sis"
import { saveHealthRecordAction } from "./actions"
...
export default async function HealthRecordPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-extrabold text-brand-navy">{displayName}</h1>
        <p className="max-w-3xl text-sm leading-relaxed text-slate-600">
          Minimum-viable health info for emergencies. Allergies, medications,
          conditions, and an out-of-band emergency contact. Office-only —
          parents don&rsquo;t see this page (they manage it via the parent
      ...
      >
        ...
        <Group title="Immunizations">
          <label className="flex items-center gap-2 text-sm text-slate-700">
            ...
            />
            <span>Immunization records on file</span>
          </label>
          <TextArea
        ...
        <Group title="Internal notes (office-only)">
          <TextArea
            label="Notes"
            name="internal_notes"
