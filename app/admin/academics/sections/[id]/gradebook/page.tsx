...
import { getCourseSectionById, type CourseSectionRecord } from "@/lib/sis"
import AcademicsHeader from "../../../AcademicsHeader"
import { periodLabel } from "../../SectionFormFields"
import {
    ...
    <div className="space-y-6">
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white px-6 py-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-600">
            ...
            categories with weights, then individual assignments. Score entry
            (the per-student grid) is coming next; the table is already in the
            DB so today&rsquo;s work won&rsquo;t need redoing.
          </p>
        </section>
          ...
          {categories.length === 0 ? (
            ...
            <div className="mt-6 space-y-2">
              {categories.map((category) => (
                <CategoryRow key={category.id} category={category} sectionId={section.id} />
              ))}
            </div>
          )}
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                ...
                an assignment visible to students and parents (once the portal
                exists).
              </p>
            </div>
...
function AssignmentRow({
  ...
  return (
    <details className="rounded-2xl border border-slate-200 bg-white">
      ...
      <div className="space-y-4 border-t border-slate-200 px-4 py-4">
        <Link
          href={`/admin/academics/sections/${sectionId}/gradebook/grade/${assignment.id}`}
